...
machine. A deployment may be bootstrapped as follows:
    ...
    python workbench-teaming-init.py [--port 8123] [--port-mongo 9123] [--port-dask 8786] [--port-dask-dashboard 8787] [--port-dask-worker 8788] [--port-dask-worker-dashboard 8789] build/pdf-dist-2021-02-10 team/pdf-dist-2021-02-10 web-host /path/to/files/on/web-host [worker1 worker2 ...]
...
def main():
    ...
    ap.add_argument('--port-dask-dashboard', default=8787, type=int,
            help="Port on which the FAW's dask dashboard may be accessed (UNSECURED!).")
    ap.add_argument('--port-dask-worker', default=8788, type=int,
            help="Port on which the FAW's dask workers may be accessed (UNSECURED!).")
    ap.add_argument('--port-dask-worker-dashboard', default=8789, type=int,
            help="Port on which the FAW's dask workers' dashboards may be accessed (UNSECURED!).")
...
port_dask_dashboard = {repr(args.port_dask_dashboard)}
port_dask_worker = {repr(args.port_dask_worker)}
port_dask_worker_dashboard = {repr(args.port_dask_worker_dashboard)}
''')
