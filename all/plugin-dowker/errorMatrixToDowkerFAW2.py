...
def errorMatrixToDowker(error_matrix, filenames, ft_names):
    ...
    for e in Edges:
        ...
        g1 = [int(v) for v in groupings[e[0]].split(',') if v]
        g2 = [int(v) for v in groupings[e[1]].split(',') if v]
        g_changed = set(g1).symmetric_difference(g2)
    ...
    for i in range(N):
        import sys
        print(groupings[i], file=sys.stderr)
        print(f'{len(ft_names)}', file=sys.stderr)
        g1 = [int(v) for v in groupings[i].split(',') if v]
        gnames = [ft_names[g] for g in g1]
