...
echo -e '#! /bin/bash\ncd /home/dist\ndask-worker --local-directory /tmp --listen-address tcp://0.0.0.0:8788 --contact-address tcp://{host.name}:{config.port_dask_worker} --dashboard-address {config.port_dask_worker_dashboard} localhost:8786 2>&1' > /etc/services.d/dask-worker/run
...
echo -e '#! /bin/bash\ncd /home/dist\ndask-worker --local-directory /tmp --listen-address tcp://0.0.0.0:8788 --contact-address tcp://{host.name}:{config.port_dask_worker} --dashboard-address {config.port_dask_worker_dashboard} {webhost}:{config.port_dask} 2>&1' > /etc/services.d/dask-worker/run
chmod a+x /etc/services.d/dask-worker/run
# Disable mongo
